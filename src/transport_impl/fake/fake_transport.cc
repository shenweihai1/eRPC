...
FakeTransport::FakeTransport(uint16_t sm_udp_port, uint8_t rpc_id, 
    ...
    : Transport(TransportType::kFake, rpc_id, phy_port, numa_node, trace_file),
      socket_fd_(-1), local_port_(sm_udp_port + 1000 + rpc_id), rx_thread_(nullptr), 
...
size_t FakeTransport::get_bandwidth() const {
  // Return 1 Gbps as default bandwidth estimate
  return 1000ULL * 1000 * 1000 / 8; // 125 MB/s
}
...
size_t FakeTransport::rx_burst() {
  ...
  while (!rx_packet_queue_.empty() && packets_processed < kPostlist) {
    ...
    // printf("DEBUG: rx_burst storing packet at ring_index=%zu, pkt_data=%p, pkt_size=%zu\n", 
    //        ring_index, pkt_data, pkt_size);
    // printf("DEBUG: rx_burst verification: rx_ring_[%zu] = %p\n", ring_index, rx_ring_[ring_index]);
    // fflush(stdout);
    
    packets_processed++;
  }
  
  // printf("DEBUG: rx_burst returning %zu packets\n", packets_processed);
  // fflush(stdout);
